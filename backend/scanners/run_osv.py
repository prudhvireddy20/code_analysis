...
def run_osv_scan(sbom_file):
    try:
        ...
        if not os.path.exists(sbom_file):
            return {
                "error": f"SBOM file not found: {sbom_file}",
                "details": "Please check the file path",
            }
        ...
        if not os.path.exists(output_file):
            return {
                "error": "OSV-Scanner did not generate output file",
                "details": "Check osv-scanner installation",
            }
        ...
        return {
            "error": "OSV-Scanner timed out after 5 minutes",
